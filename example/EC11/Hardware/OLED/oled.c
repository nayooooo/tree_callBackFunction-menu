...
{
	u8 pos,bx,temp=0;
	if(x>=OLED_COL_MAX||y>=OLED_ROW_MAX)return;
	pos=7-y/8;
	bx=y%8;
	temp=1<<(7-bx);
...
u8 OLED_ReadPoint(u8 x,u8 y)
{
	u8 pos,bx,temp=0;
	u8 point_State;
	if(x>=OLED_COL_MAX||y>=OLED_ROW_MAX)return -1;
	pos=7-y/8;
	bx=y%8;
	temp=1<<(7-bx);
	if(OLED_GRAM[x][pos]&temp)point_State=OLED_POINT_WHITE;
	else point_State=OLED_POINT_BLACK;
	return point_State;
}
...
void OLED_ShowDecimal(u8 x, u8 y, u32 Int_Part, u8 Dec_Part, u8 size, u8 mode)
{
	...
	if(size==8) OLED_ShowChar(x+digital*size, y, '.', size, mode);
	else OLED_ShowChar(x+digital*(size/2), y, '.', size, mode);
	if(size==8) OLED_ShowNums(x+(digital+1)*size, y, Dec_Part, size, mode);
	else OLED_ShowNums(x+(digital+1)*(size/2), y, Dec_Part, size, mode);
}
